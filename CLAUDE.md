...
`one-system-{dark,light}.svg` (1280×680), the page's only graphic: badge kicker,
two-tone gradient "See it as one system" headline, the six pinned repos as bordered mono
chips converging by gradient beams into one node, a stem down to a sanlee.me pill, and
the NET OPS → SOFTWARE → PRODUCT circuit trace as a small signature in the top-right
corner (inherited from the retired banner). Subtle CSS motion (packet streaks on the
beams, junction pulse, kicker-dot blink) runs even inside GitHub's proxied `<img>` and
is fully disabled under `prefers-reduced-motion`. The README wraps the image in a link
to the site (SVGs rendered through `<img>` cannot carry live links themselves).
